...
const mapCustomerContactListItemFromDb = (contact: DbCustomerContact): CustomerContactListItem => {
...
export const BusFlowApi = {
    ...
    async getCustomerContactsList(params: CustomerContactListParams): Promise<CustomerContactListResult> {
        const query = (params.query || '').trim();
        const from = Math.max(0, (params.page - 1) * params.pageSize);
        const to = from + params.pageSize - 1;
        const ilikeQuery = `%${escapeIlikeValue(query)}%`;
    ...
    async createCustomerContactWithCompany(payload: {
        companyName: string;
        firstName?: string;
        lastName?: string;
        fullName?: string;
        ...
        const created = await this.upsertCustomerContact({
    ...
    async updateCustomerContact(
        ...
        const updated = await this.upsertCustomerContact({
            id: contactId,
            customerId,
