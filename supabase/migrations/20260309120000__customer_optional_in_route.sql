-- Make customer optional in save_busflow_route_with_stops RPC.
-- Previously a customer_id was required for every route save, which blocked
-- newly registered accounts (empty customer list) from creating routes.
...
BEGIN
  ...
  -- Customer is optional. Validate only when provided.
  ...
  IF v_customer_contact_id IS NOT NULL THEN
    ...
    IF v_customer_contact_id IS NOT NULL AND v_customer_id IS NULL THEN
